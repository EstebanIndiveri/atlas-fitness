...
- Rutinas custom: `POST/PATCH/DELETE /api/routines` (y `PATCH /api/routines/[id]`). Mutar sistema → 403. `routine_exercises` se reemplaza en transacción; `sort_order` único. Media de ejercicios: URLs en el PATCH existente (`imageUrl`/`videoUrl`).
