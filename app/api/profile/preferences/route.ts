 ...
 * @returns Stored preference IDs, or null for each field when none are saved.
 * @example
 * await GET(request);
 */
 ...
 * @returns The validated preference IDs persisted for the authenticated user.
...
export async function PUT(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    const preferences = await saveUserPreferences(session.userId, body);
    return NextResponse.json(preferences);
  } catch (error) {
    return handleApiError(error);
  }
